...
# ═══════════════════════════════════════════════════════════════════════════════
# 4. Обратная совместимость: Trip.consignor / Trip.consignee
# ═══════════════════════════════════════════════════════════════════════════════

class ConsignorConsigneeSyncTests(RouteBuilderTestBase):
    """
    Тесты синхронизации Trip.consignor / Trip.consignee.

    Устаревшие поля Trip заполняются автоматически при сохранении:
    - consignor = organization первой LOAD-точки (по sequence)
    - consignee = organization первой UNLOAD-точки (по sequence)

    Это обеспечивает обратную совместимость с отчётами, шаблонами ТН,
    списком рейсов и другими частями системы, которые читают эти поля.
    """

    def test_consignor_from_first_load_point(self):
        """
        consignor берётся из organization первой LOAD-точки.
        Если LOAD-точек несколько, берётся та, что первая по sequence (= по порядку массива).
        """
        points = [
        ...
        trip = Trip.objects.order_by("-pk").first()
        self.assertEqual(trip.consignor_id, self.our_org.pk,
                         "consignor должен быть organization первой LOAD-точки")

    def test_consignee_from_first_unload_point(self):
        """
        consignee берётся из organization первой UNLOAD-точки.
        """
        points = [
        ...
        trip = Trip.objects.order_by("-pk").first()
        self.assertEqual(trip.consignee_id, self.third_org.pk,
                         "consignee должен быть organization первой UNLOAD-точки")

    def test_consignor_null_when_no_org(self):
        """
        Если на LOAD-точках не указана организация,
        Trip.consignor должен быть NULL.
        """
        points = [
            self._make_point("LOAD", "Адрес-1", organization=""),
            self._make_point("UNLOAD", "Адрес-2"),
        ]
        resp = self._post_with_points(points)
        self.assertEqual(resp.status_code, 302)

        trip = Trip.objects.order_by("-pk").first()
        self.assertIsNone(trip.consignor_id)
        self.assertIsNone(trip.consignee_id)

    def test_sync_updates_on_edit(self):
        """
        При редактировании рейса consignor/consignee пересчитываются
        из новых данных точек. Старые значения перезаписываются.
        """
        # Создаём с org_1 как отправитель
        points = [
            self._make_point("LOAD", "А", organization=self.our_org.pk),
            self._make_point("UNLOAD", "Б"),
        ]
        resp = self._post_with_points(points)
        trip = Trip.objects.order_by("-pk").first()
        ...
        # Редактируем — меняем отправителя на third_org
        new_points = [
            self._make_point("LOAD", "А", organization=self.third_org.pk),
            self._make_point("UNLOAD", "Б", organization=self.external_org.pk),
        ]
        resp = self._post_with_points(
            new_points,
            url=reverse("trips:edit", kwargs={"pk": trip.pk}),
        ...
        trip.refresh_from_db()
        self.assertEqual(trip.consignor_id, self.third_org.pk,
                         "consignor должен обновиться при редактировании")
        self.assertEqual(trip.consignee_id, self.external_org.pk,
                         "consignee должен обновиться при редактировании")


# ═══════════════════════════════════════════════════════════════════════════════
# 5. Валидация: отклонение некорректных данных
# ═══════════════════════════════════════════════════════════════════════════════
