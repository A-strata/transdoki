...
from organizations.models import UserOwnedModel, Organization
...
class Waybill(UserOwnedModel):
    ...
    def clean(self):
        ...
        if self.pk:
            original = type(self).objects.filter(
                pk=self.pk).only('date').first()
            if original and original.date != self.date:
                raise ValidationError({
                    'date': ('Нельзя изменять дату после '
                             'создания путевого листа.')
                })
