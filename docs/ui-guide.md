...
**7. CSS и JS — внешние файлы, не инлайн**

Стили и скрипты хранятся во внешних файлах и подключаются через `{% static %}`. Это стандартный Django-way: файлы кешируются браузером и корректно подхватываются `collectstatic`.

**Структура:**
```
static/
  css/forms.css        # общие компоненты форм (.field, .errorlist и др.)
  css/nav.css          # навбар
...
**Правила:**
- Повторяющиеся стили/скрипты (2+ шаблона) → внешний файл в `static/`
- Специфичные для одного шаблона → `static/<app>/css/` или `static/<app>/js/`, подключить через `{% block extra_js %}`
- Инлайн `<style>` и `<script>` — только если в код нужно передать данные из Django-контекста (например, `{% url '...' %}` внутри JS). В этом случае предпочтительнее передавать данные через `data-`атрибуты в HTML и читать их из внешнего JS-файла

**Пример передачи URL через data-атрибут:**
```html
<div id="inn-lookup" data-url="{% url 'organizations:api_suggestions_by_inn' %}"></div>
```
```javascript
// в внешнем .js файле:
const url = document.getElementById('inn-lookup').dataset.url;
```
